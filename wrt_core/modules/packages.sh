...
remove_unwanted_packages() {
    ...
    local packages_net=(
        ...
        "mosdns" "ddns-go" "adguardhome" "naiveproxy" "shadowsocks-rust"
...
fix_argon_wget_depends() {
    local dir="$(get_custom_feed_worktree_dir)/luci-theme-argon"
    local patch_file="$BASE_PATH/patches/021-argon-wget-depends.patch"
    [ -d "$dir" ] || return 0
    if grep -q '+wget-any' "$dir/Makefile" 2>/dev/null; then
        if patch --dry-run -p1 -d "$dir" -i "$patch_file" >/dev/null 2>&1; then
            patch -p1 -d "$dir" -i "$patch_file" && echo "[argon] wget 依赖已修复"
        fi
    fi
}
...
update_smartdns() {
    ...
    sed -i 's/^PKG_MIRROR_HASH:=.*/PKG_MIRROR_HASH:=skip/' "$SMARTDNS_DIR/Makefile"
