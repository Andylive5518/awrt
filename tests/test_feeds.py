...
""",
            ...
            self.assertNotIn("config.AllowInsecure", fixed)
            self.assertIn("+\t\tconfig.Fingerprint = \"unsafe+\" + config.Fingerprint", fixed)
            self.assertIn("--- a/transport/internet/tls/config.go", fixed)
            self.assertIn("strings.HasPrefix(c.Fingerprint, \"unsafe+\")", fixed)
            self.assertIn("+\t\tconfig.InsecureSkipVerify = true", fixed)
            self.assertIn("--- a/transport/internet/tls/tls.go", fixed)
            self.assertIn("strings.TrimPrefix(name, \"unsafe+\")", fixed)
